...
if(isset($_GET['type']) && $_GET['type']=='update' && isset($_GET['id'])){
	...
	mysqli_query($con,"update `faculty_leave` set leave_status='$status'and where id='$id'");
}
if(($_SESSION['ROLE']==1) && ($_SESSION['DEPARTMENT_ID'])){ 
	$sql="select `leave`.*, employee.name,employee.role=2  as eid  from `leave`,employee where `leave`.employee_id=employee.id order by `leave`.id desc";
}else{
                              ...
                              <table class="table ">
                                 ...
                                 <tbody>
									...
									<tr>
                                       ...
                                       <td><?php  $date=date_create($row['leave_from']);
										 echo date_format($date,"d/m/Y")?></td>
									   <td><?php
										$date=date_create($row['leave_to']);
										 echo date_format($date,"d/m/Y")?></td>
